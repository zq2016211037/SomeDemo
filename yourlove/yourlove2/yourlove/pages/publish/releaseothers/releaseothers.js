...
Page({
	...
	onLoad: function(options){
		...
		if(statusId=='trade'){
        status = 1
	    } else if(statusId == 'experience'){
	        status = 2
	    }else if(statusId == 'topic'){
	        status = 3
	    }else if(statusId == 'advertise'){
	        status = 4
	    }else{
	        status = 5
	    };
	...
	releaseLove: function(){
		...
		console.log(typeof(filenameall))
		wx.request({
			url: "http://120.79.94.90:8080/yourlove/topic/addTopic",
			data:{
				userId: userId,
				title: title,
				content: content,
